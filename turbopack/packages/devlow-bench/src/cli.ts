...
interface RunOptions {
  ...
  n?: number
  warmup?: number
  // Path override for the always-on snapshot CSV. Undefined → default path.
  snapshot?: string
  compare?: boolean
  baseline?: string
  json?: string
...
;(async () => {
  ...
  program
    .command('compare')
    .description(
      'Compare two snapshot CSVs side-by-side, with mean/p50/p90 plus Welch’s t and Mann–Whitney U p-values per metric.'
    )
...
async function runRun(
  ...
  for (const pair of opts.define ?? []) {
    const eq = pair.indexOf('=')
    if (eq === -1) {
      console.error(`devlow-bench: invalid -D ${pair} (expected key=value).`)
      process.exit(1)
    }
  ...
  const cliIface = {
    filterScenarios: async (scenarios: Scenario[]) => {
      ...
      return scenarios.filter((s) => filters.some((f) => s.name.includes(f)))
    },
    filterScenarioVariants: async (variants: ScenarioVariant[]) => {
      if (propFilters.length === 0) return variants
      // Group multiple -D key=value with the same key into an OR set.
