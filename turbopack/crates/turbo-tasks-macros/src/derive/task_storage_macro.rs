...
fn generate_typed_storage_struct(grouped_fields: &GroupedFields) -> TokenStream {
    ...
    let lazy_field = if has_lazy {
        // Bitmap-driven byte tail. `LazyTail` owns the heap buffer holding
        // packed lazy-field payloads plus the `present` bitmap (one bit per
        // variant tag) and `len`/`cap` byte counters. Total slot size is
        // `size_of::<LazyTail>() == 16 B` on 64-bit, matching the previous
        // `TinyVec<LazyField, _>` slot exactly.
        quote! {
